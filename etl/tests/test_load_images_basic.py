...
class TestImageLoadServiceBasic:
    @pytest.fixture
    def raw_data(self):
    ...
    def test_get_records_needing_processing_never_loaded(
    ...
    def test_get_records_needing_processing_museum_filter(self, service):
        ...
        assert len(smk_records) >= 0
        assert len(cma_records) == 0

    def test_should_process_image_missing_from_s3(
        self, service, transformed_data, mock_bucket_service
    ):
        """Test processing when image is missing from S3."""
        # Set current hash to match thumbnail URL (so hash doesn't change)
    ...
    def test_should_not_process_when_s3_exists_and_no_hash_change(
        self, service, transformed_data, mock_bucket_service
    ):
        """Test skipping when image exists and no hash change detected."""
        # Set up the record to have a hash that matches current URL
        try:
            current_hash = hashlib.sha256(
                transformed_data.thumbnail_url.encode()
            ).hexdigest()
            ...
            # thumbnail_url_hash field doesn't exist, skip this test
            pytest.skip("thumbnail_url_hash field not available")
    ...
    def test_process_single_record_success(
        self, service, transformed_data, mock_bucket_service
    ):
    ...
    def test_process_single_record_error(
        self, service, transformed_data, mock_bucket_service
    ):
    ...
    def test_run_batch_processing_with_records(
        self, service, transformed_data, mock_bucket_service
    ):
        ...
        stats = service.run_batch_processing(batch_size=10)
