...
class ImageLoadService:
    ...
    def get_records_needing_processing(
        ...
        """
        query = Q(image_loaded=False) | ~Q(source_raw_hash=models.F('raw_data__raw_hash'))
        
        if museum_filter:
            query &= Q(museum_slug=museum_filter)
        
        return TransformedData.objects.filter(query).select_related('raw_data')[:batch_size]
    ...
    def should_process_image(self, record: TransformedData) -> tuple[bool, str]:
        ...
        if record.thumbnail_url_hash != current_hash:
            return True, f"thumbnail_url_hash changed (old: {record.thumbnail_url_hash}, new: {current_hash})"
