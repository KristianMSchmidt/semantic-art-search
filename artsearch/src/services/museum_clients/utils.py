...
def get_museum_api_url(
    ...
    if museum_slug in ("met", "rma"):
        assert museum_db_id is not None, "museum_db_id is required for MET and RMA API URLs"
...
def get_museum_page_url(
    ...
    museum_api_client = get_museum_client(museum_slug)
    if museum_slug == "met":
        assert museum_db_id is not None, "museum_db_id is required for MET page URLs"
